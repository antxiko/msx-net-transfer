...
/*
   ...
   NOTE (msx-net-transfer fork): All function declarations in this file carry __sdcccall(0) to
   match the stack-based calling convention used in unapi_tcp.asm. The project is compiled with
   SDCC's default sdcccall 1 (register-based), so this override is necessary for UNAPI asm
   routines to receive the correct pointer arguments. Variable declarations (active_implementation
   etc.) are unchanged — __sdcccall(0) is a function attribute only.
   This file shadows MSXgl's engine/src/network/unapi_tcp.h; keep it in sync with the original.
*/
#pragma once

// TCP/IP UNAPI management
extern	int	tcpip_enumerate(void) __sdcccall(0);	// performs checking for installed implementation and gets calling information for 4 of them, returns number of implementations (max 4). Resets active_implementation to 0
...
extern	int	tcpip_impl_getinfo(char** impl_string,	// UNAPI_GET_INFO: Obtain the implementation name and version. The implementation
			...
			int* api_impl_version) __sdcccall(0);	// is wrong (not within 0..number of implementations-1) then ERR_INV_IMPL is returned
...
extern	int	tcpip_get_capab_flags_llproto(tcpip_unapi_capab_flags_llproto* capab_flags_llproto) __sdcccall(0);
extern	int	tcpip_get_capab_connections(tcpip_unapi_capab_connections* capab_connections) __sdcccall(0);
extern	int	tcpip_get_capab_dtg_sizes(tcpip_unapi_capab_dtg_sizes* capab_dtg_sizes) __sdcccall(0);
...
extern	int	tcpip_get_ipinfo(tcpip_unapi_ip_info* ip_info) __sdcccall(0);

/* TCPIP_NET_STATE: Get network state */
extern	int	tcpip_net_state(int* net_state) __sdcccall(0);
...
extern	int	tcpip_send_echo(tcpip_unapi_echo* echo_param_block) __sdcccall(0);

/* TCPIP_RCV_ECHO: Retrieve ICMP echo response message. The call populates
   the structure at the address passed
 */
extern	int	tcpip_rcv_echo(tcpip_unapi_echo* echo_param_block) __sdcccall(0);
...
extern	int	tcpip_dns_q(char* hostname, tcpip_unapi_dns_q* dns_q) __sdcccall(0);
...
extern	int	tcpip_dns_s(tcpip_unapi_dns_q* dns_q) __sdcccall(0);

/***** UDP protocol related routines *****/

/* TCPIP_UDP_OPEN: Open a UDP connection */
extern	int	tcpip_udp_open(int port_number, int conn_lifetime, int* conn_number) __sdcccall(0);

/* TCPIP_UDP_CLOSE: Close a UDP connection */
extern	int	tcpip_udp_close(int conn_number) __sdcccall(0);
...
extern	int	tcpip_udp_state(int conn_number, tcpip_unapi_udp_state* udp_state) __sdcccall(0);

/* TCPIP_UDP_SEND: Send an UDP datagram. Destination IP address, destination
   port number and data size fields must be populated */
extern	int	tcpip_udp_send(int conn_number, tcpip_unapi_udp_dtg_parms* dtg_parms, char* data) __sdcccall(0);

/* TCPIP_UDP_RCV: Retrieve an incoming UDP datagram. Populates IP address,
   source port and actual received data length fields */
extern	int	tcpip_udp_rcv(int conn_number, char* buffer, int dtg_maxsize, tcpip_unapi_udp_dtg_parms* dtg_parms) __sdcccall(0);
...
extern	int	tcpip_tcp_open(tcpip_unapi_tcp_conn_parms* tcp_conn_params, int* conn_number) __sdcccall(0);

/* TCPIP_TCP_CLOSE: Close a TCP connection */
extern	int	tcpip_tcp_close(int conn_number) __sdcccall(0);

/* TCPIP_TCP_ABORT: Abort a TCP connection
 */
extern	int	tcpip_tcp_abort(int conn_number) __sdcccall(0);
...
/* TCPIP_TCP_STATE: Get the state of a TCP connection. Puts information into connection state,
   ...
   numbers in */
extern	int	tcpip_tcp_state(int conn_number, tcpip_unapi_tcp_conn_parms* tcp_conn_params) __sdcccall(0);
...
extern	int	tcpip_tcp_send(int conn_number, char* data, int data_length, int flags) __sdcccall(0);

/* TCPIP_TCP_RCV: Receive data from a TCP connection. Updates both received byte numbers
   within the structure */
extern	int	tcpip_tcp_rcv(int conn_number, char* buffer, int maxsize, tcpip_unapi_tcp_conn_parms* tcp_conn_params) __sdcccall(0);

/* TCPIP_TCP_FLUSH: Flush the output buffer of a TCP */
extern	int	tcpip_tcp_flush(int conn_number) __sdcccall(0);
...
extern	int	tcpip_ipraw_open(int protocol_number, int conn_lifetime, int* conn_number) __sdcccall(0);

/* TCPIP_RAW_CLOSE: Close a raw IP connection */
extern	int	tcpip_ipraw_close(int conn_number) __sdcccall(0);

/* TCPIP_RAW_STATE: Get the state of a raw IP connection */
extern	int	tcpip_ipraw_state(int conn_number, tcpip_unapi_ipraw_state* ipraw_state) __sdcccall(0);
...
extern	int	tcpip_ipraw_send(int conn_number, char* data, tcpip_unapi_ipraw_parms* ipraw_params) __sdcccall(0);

/* TCPIP_RAW_RCV: Retrieve an incoming raw IP datagram
 */
extern	int	tcpip_ipraw_rcv(int conn_number, char* buffer, int maxsize, tcpip_unapi_ipraw_parms* ipraw_params) __sdcccall(0);

/***** Configuration related routines*****/

/* TCPIP_CONFIG_AUTOIP: Enable or disable the automatic IP addresses retrieval */
extern	int	tcpip_config_autoip_get(char* ip_mode) __sdcccall(0);
extern	int	tcpip_config_autoip_set(char ip_mode) __sdcccall(0);

/* TCPIP_CONFIG_IP: Manually configure an IP address
 */
extern	int	tcpip_config_ip(tcpip_unapi_ip_info* ip_info) __sdcccall(0);

/* TCPIP_CONFIG_TTL: Get/set the value of TTL and TOS for outgoing datagrams */
extern	int	tcpip_config_ttltos_get(unsigned* ttltos_get) __sdcccall(0);
extern	int	tcpip_config_ttltos_set(unsigned ttltos_set) __sdcccall(0);

/* TCPIP_CONFIG_PING: Get/set the automatic PING reply flag */
extern	int	tcpip_config_ping_get(char* flag_value_get) __sdcccall(0);
extern	int	tcpip_config_ping_set(char flag_value_set) __sdcccall(0);
