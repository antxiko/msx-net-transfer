...
static u8 Net_Send(NetConn conn, const u8* data, u16 length)
{
    ...
    // [FIX INL 2026-06-08] NO tcpip_tcp_flush: la funcion UNAPI #19 es TCPIP_TCP_DISCARD
    // (borra el buffer de salida AUN NO enviado), NO un flush. En INL (envio diferido al
    // processing step del timer) borraba la respuesta antes de transmitirla. El push=1 del
    // send ya fuerza el envio. GR8NET/UnapiNet no se veian afectados (envian en la llamada).
    // Fix identico al de client/network.h. Ver MEMORIA_FIX_NT_ObsoNET.md.
